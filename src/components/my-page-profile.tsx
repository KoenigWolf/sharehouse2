...
function FieldRow({ label, value }: { label: string; value: string | null | undefined }) {
  if (!value) return null;
  return (
    <div className="py-3 border-b border-[#f5f5f3] last:border-0">
      <dt className="text-[10px] text-[#a3a3a3] tracking-wide mb-1.5">{label}</dt>
...
export function MyPageProfile({ profile, teaTimeEnabled, roomPhotos }: MyPageProfileProps) {
  const t = useI18n();
  ...
  return (
    ...
    >
      ...
      {/* Cover Photo */}
      <m.div variants={itemVariants} className="bg-white border border-[#e5e5e5] overflow-hidden">
        <CoverPhotoSection
          coverPhotoUrl={coverUrl}
          onUpload={handleCoverUploadClick}
      ...
      {roomPhotos.length > 0 && (
        <m.div variants={itemVariants} className="mt-4">
          <ProfileSection title={t("roomPhotos.roomPhotosSection")} category="photos">
