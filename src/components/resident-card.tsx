...
export const ResidentCard = memo(function ResidentCard({
  ...
  const displayName = isBlurred ? maskedName : (profile.nickname || profile.name);
  ...
  const cardContent = (
    <m.article
      ...
      whileTap={isBlurred ? undefined : { scale: 0.97 }}
      transition={{ type: "spring", stiffness: 400, damping: 25 }}
      className={`
        relative aspect-[3/4] rounded-2xl overflow-hidden
        bg-muted
        shadow-md shadow-black/10
        ${!isBlurred ? "group-hover:shadow-xl group-hover:shadow-black/15" : ""}
