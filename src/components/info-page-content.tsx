  ...
  return (
    <div className="bg-white border border-[#e5e5e5] p-4 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[10px] text-[#a3a3a3] tracking-wide uppercase">{label}</p>
        <p className="text-sm text-[#1a1a1a] font-medium font-mono truncate">{value}</p>
    ...
    <div className="bg-white border border-[#e5e5e5]">
      <div className="px-4 py-3 border-b border-[#e5e5e5] bg-[#fafaf8]">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] text-[#a3a3a3] tracking-wide uppercase">
              {t("wifi.password")}
            </p>
            <p className="text-sm text-[#1a1a1a] font-mono mt-0.5">
        ...
        {wifiInfos.map((wifi) => (
          <div key={wifi.id} className="px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="text-[10px] text-[#a3a3a3] w-14">{wifi.area_name}</span>
              <span className="text-sm text-[#1a1a1a] font-mono">{wifi.ssid}</span>
            </div>
          </div>
        ))}
      </div>
          ...
          <div className="space-y-3">
            {addressInfo && (
              <div className="bg-white border border-[#e5e5e5] p-4">
                ...
                <p className="text-sm text-[#1a1a1a]">{addressInfo.content}</p>
                {addressInfo.notes && (
                  <p className="text-xs text-[#737373] mt-2">{addressInfo.notes}</p>
                )}
              </div>
            )}
            {sharedInfos
              ...
              .map((info) => (
                <div key={info.id} className="bg-white border border-[#e5e5e5] p-4">
                  ...
                  <p className="text-sm text-[#1a1a1a]">{info.content}</p>
                  {info.notes && (
                    <p className="text-xs text-[#737373] mt-2">{info.notes}</p>
                  )}
                </div>
              ))}
          </div>
