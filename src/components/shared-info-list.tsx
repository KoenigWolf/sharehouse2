  ...
  if (sharedInfos.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-[#636861]">情報がありません</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {sharedInfos.map((info) => (
        ...
        >
          <p className="text-[10px] text-[#959892] tracking-wide uppercase mb-2">
            {info.title}
          </p>
          <p className="text-sm text-[#272a26] font-medium tracking-wide">
            {info.content}
          </p>
          {info.notes && (
            <p className="text-xs text-[#636861] mt-2">
              {info.notes}
            </p>
          )}
        </div>
