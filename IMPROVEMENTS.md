...
- `console.error` のみの現状を構造化ログに改善
- `logError` ユーティリティを作成し、将来的にSentry等と統合
- エラー境界でNext.jsのdigestを活用

### 2.4 型安全性の向上

- `use-async.ts` の依存配列にオブジェクト参照が含まれ無限ループのリスクあり
- `options` を分解して個別プロパティを依存配列に指定

### 2.5 フォーム最適化

- `profile-edit-form.tsx` のフォームが1つのオブジェクトstateに集約されており、1入力変更で全体が再レンダリング
- React 19 の `useFormState` / `useFormStatus` の導入、または個別state分割を検討
...
- 対象: `auth/actions.ts`, `profile/actions.ts`, `tea-time/actions.ts`
- Supabaseクライアントのモックを使ったユニットテスト
...
- **モーション配慮**: `prefers-reduced-motion` メディアクエリ対応
